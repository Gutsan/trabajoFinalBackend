...
export class productModels {
    ...
    static async getProductForCategory(category_id, Limit, Offset) {
        const query = 'SELECT * FROM product WHERE category_id=$1 LIMIT=$2 OFFSET=$3'
        const values = [category_id, Limit, Offset]
        const { rows } = await pool.query(query, values)
        return rows
    }
    static async createProduct(name, description, img_url, price, stock, category_id) {
        const query = "INSERT INTO product (id,name, description, img_url, price, stock, category_id) VALUES (DEFAULT,$1,$2,$3,$4,$5,$6)"
